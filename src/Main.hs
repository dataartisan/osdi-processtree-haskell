...
printProcessTree (header : lines) = 
  let procs = map (parser header) lines
      pmap = IntMap.fromList $ map (\(Proc(pid, _, cmd)) -> (pid, cmd)) procs
      tmap = IntMap.fromListWith (++) $ map (\(Proc(pid, ppid, _)) -> (ppid, [pid])) procs
      showTrees l i = concatMap (showTree l) (tmap ! i)
      showTree l i = ((concat $ replicate l " ") ++ (show i) ++ ": " ++ (pmap ! i)) :
